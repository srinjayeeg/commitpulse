...
function renderTopRepos(
  repos: Array<{ name: string; stars: number; language: string | null }>,
  text: string,
  accent: string,
  sf: number,
  _W: number,
  ...
  const x = Math.round(24 * sf);
  const rowH = Math.round(42 * sf);
  const startY = Math.round(H / 2) - Math.round((repos.length * rowH) / 2);
  const labelFont = Math.round(8 * sf);
  const nameFont = Math.round(11 * sf);
  const dotR = Math.round(3 * sf);
  const header = `<text x="${x}" y="${startY - Math.round(16 * sf)}" fill="${accent}" font-size="${labelFont}" font-family="${font.replace(/"/g, '')}" letter-spacing="2" font-weight="600">TOP REPOS</text>`;
  const rows = repos
    .map((repo, i) => {
      ...
      const langColor = lang ? LANGUAGE_COLORS[lang] || '#a855f7' : accent;
      const dotX = x + dotR;
      const langX = dotX + dotR + Math.round(6 * sf);
      const langMarkup = lang
        ? `<circle cx="${dotX}" cy="${y + Math.round((nameFont + 6) * sf)}" r="${dotR}" fill="${langColor}" /><text x="${langX}" y="${y + Math.round((nameFont + 8) * sf)}" fill="${text}" font-size="${labelFont}" font-family="${font.replace(/"/g, '')}" letter-spacing="1">${escapeXML(lang.toUpperCase())}</text>`
        : '';
      return `<g>
      <text x="${x}" y="${y + Math.round(nameFont * sf)}" fill="${accent}" font-size="${nameFont}" font-family="${font.replace(/"/g, '')}" letter-spacing="2" font-weight="600">${safeName}</text>
      ${langMarkup}
    </g>`;
    })
    .join('');
...
<svg xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 ${W} ${H}" fill="none" role="img" aria-labelledby="cp-title-${safeId}" aria-describedby="cp-desc-${safeId}">
  ...
  ${params.show_repos && params.topRepos ? renderTopRepos(params.topRepos, text, mainAccentHex, sf, W, H, statsFont) : ''}
</svg>`;
}
